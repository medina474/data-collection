create function abonner(_adherent_id bigint, _panier_id bigint, _distribution_id bigint, _date date, _quantite int default null)
 returns bigint
 language plpgsql
as $function$
declare
...
begin
  ...
  select coalesce(_quantite, quantite), produit_id into _nombre, _produit_id
  	from paniers
...
-- Insérer l'abonnement
  ...
  select _adherent_id, panier_id, prix, _quantite, _date_debut, _saison_id
    from paniers
    where panier_id = _panier_id
...
une_proposition as (
	...
	where frequence_id = 1
  	and calendrier_id = _calendrier_id
  	and jour between '2025-01-01' and '2025-12-31'
),
final_selection as (
