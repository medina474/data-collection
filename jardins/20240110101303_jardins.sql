...
grant usage on schema jardins to role_web;
alter default privileges in schema jardins grant select on tables to role_web;
...
create unique index jardins_pk
  on jardins.jardins
  using btree (jardin_id);

alter table jardins.jardins
  add primary key using index jardins_pk;
